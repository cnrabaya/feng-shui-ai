...
export function useRoomAnalysis() {
   ...
   */
  const runAnalysis = useCallback(async ({ imageBase64, imageMediaType, width, height, unit }) => {
    update({ step: ANALYSIS_STEPS.ANALYZING, error: null, progress: 'Reading the room…', analysis: null, layouts: null });

    try {
      const analysis = await analyzeRoom({ imageBase64, imageMediaType, width, height, unit });
